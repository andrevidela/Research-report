...
developement spawned by the work of Girard on Linear logic. Until
Idris2, our understanding of linear types and their uses in
dependently-typed programs was hypothetical. However this changes with
languages like Idris2, which allow us to explore the semantics of
running programs using linear and dependent types. In this thesis I
explore multiple facets of programming through the lens of quantitative
programming, from ergonomics, to performance. I will present how
quantitative annotations can help the programmer write program that
precisely match their intension, help the compiler better analyse the
program written, and help the output bytecode to run faster.
...
ergonomics of Quantitative Type Theory (QTT, for short) for software
development and the performance improvement we can derive from the
linearity features of Idris2.
...
\hypertarget{mapping-primitive-to-data-types-and-vice-versa-todo-spin-this-as-a-motivation-for-automatically-deriving-instances-not-just-copy-and-drop}{%
\subsection{Mapping primitive to data types and vice-versa (TODO: Spin
this as a motivation for automatically deriving instances, not just Copy
and
Drop)}\label{mapping-primitive-to-data-types-and-vice-versa-todo-spin-this-as-a-motivation-for-automatically-deriving-instances-not-just-copy-and-drop}}
...
classical constructors. However, we could define \texttt{String} as a
list of \texttt{Char}, and have \texttt{Char} be defined as an 8 bits
vector. Using both those definitions our primitive types have now become
plain data with regular constructors.
...
This allows us to implement \texttt{Copy} and \texttt{Drop} as expected.
...
Typedefs, which describe types and allow operations on the descriptions
without modifying the semantics of the structure described, all while
being able to map them to primitive version of themselves.
...
\section{Idris2 and multiplicities}\label{idris2-and-multiplicities}}
...
\section{2 Implementation strategy}\label{implementation-strategy}}
...
\section{3 Benchmarks \& methodology}\label{benchmarks-methodology}}
